/*
  Скетч к проекту "Универсальный пульт для шлагаумов и люстр RF 433.96MHz / 315MHz с OLED дисплеем и хранением 30 ключей в памяти EEPROM"
...
#define G_Led 13        // встроенный светодиод
...
enum emSnifferMode
{
  ...
  smdAutoRec,
  smdAutoRecSilence
} snifferMode; // режим работы сниффера
...
void OLED_printKey(tpKeyData *kd, byte msgType = 0)
{
  ...
  case smdNormal:
    myOLED.clrScr();
    myOLED.print("N", RIGHT, 24);
    break;
  case smdAutoRec:
    myOLED.clrScr();
    myOLED.print("A", RIGHT, 24);
    break;
  case smdAutoRecSilence:
    return;
  }
...
void OLED_printError(String st, bool err = true)
{
  ...
  if (err)
    myOLED.print(F("Error!"), 0, 0);
  else
    myOLED.print(F("OK"), 0, 0);
...
{ // возвращает индекс или ноль если нет в ROM
  bool eq = true;
  byte *buf = (byte *)kd;
  ...
  { // ищем ключ в eeprom.
    byte i = 0;
    if ((kd->type == kKeeLoq) || (kd->type == kANmotors64))
      i = 4; // для эих ключей первая часть кода переменная
    for (; i < kd->codeLenth >> 3; i++)
      if (buf[i] != EEPROM[i + j * sizeof(tpKeyData)])
      {
        eq = false;
        break;
      }
...
void setup()
{
  ...
  pinMode(RVR_Vcc_Pin, OUTPUT);
  digitalWrite(RVR_Vcc_Pin, HIGH); // включение питания приемника
  pinMode(TM_gnd_Pin, OUTPUT);
  digitalWrite(TM_gnd_Pin, LOW); // земля передатчика
  pinMode(TM_Vcc_Pin, OUTPUT);
  digitalWrite(TM_Vcc_Pin, LOW); // питание передачика отключено
  pinMode(txPin, OUTPUT);
  digitalWrite(txPin, LOW);     // вход передатчика
  ...
  attachInterrupt(digitalPinToInterrupt(Btn_ok_Pin), wakeUp, CHANGE); // проснемся от нажатия кнопки OK
...
void handleInt()
{
  ...
  if (((duration < minPause) || (duration > maxPause)) && (changeCnt == 0))
    return; // ждем начальный синхроимпульс

  if ((duration > minPause) && (duration < maxPause) && (changeCnt >= 24))
  { // похоже на очередной синхроимпульс
    const int delta = duration - abs(keyRawLog[0]);
    if (abs(delta) < (350 >> prescal))
    { // это точно повторный сигнал
      repeatCnt++;
      if (repeatCnt >= 2)
      {
        recieved = true;
  ...
  if ((duration > minPause) && (duration < maxPause) && (changeCnt > 0) && (changeCnt < 20))
  { // очередной синхроимпульс но мало полученных данных
    changeCnt = 0;
    repeatCnt = 0;
  }

  if ((repeatCnt > 0) && (abs(duration - abs(keyRawLog[changeCnt])) > (100 >> prescal)))
  { // сравниваем с предыдущей посылкой
    changeCnt = 0;
    repeatCnt = 0;
    return;
  }
...
bool convert2Key(tpKeyData *kd)
{
  ...
  if (logLen > 131)
  { // Keeloq 64 bit коды со случайным сегментом и длинной преамулой
    for (; i < logLen; i++)
    {
      if (abs(keyRawLog[i]) > (2000 >> prescal))
        break; // находим второй синхроимпульс
  ...
  for (byte k = 0; k < 4; k++)
  {
    ...
    for (byte i = 0; i < kd->codeLenth; i++)
    {
      if (((i >> 3) >= 2) && ((i >> 3) <= 3) && (kd->type == kANmotors64))
        bt = 1 & (ANmotorsByte >> (7 - j)); // заменяем 2 и 3 айты на случайное число для ANmotors
      else
...
void printDebugData()
{
  ...
  Serial.print(F(", zero ["));
  Serial.print(keyData1.zero[0] << prescal);
  Serial.print(", ");
  Serial.print(keyData1.zero[1] << prescal);
  btn_ok.tick();
  btn_left.tick();
  btn_right.tick();
  Serial.print(F("], one ["));
  Serial.print(keyData1.one[0] << prescal);
  Serial.print(", ");
  Serial.print(keyData1.one[1] << prescal);
  Serial.print("]");
  Serial.print(F(", startPause ["));
  Serial.print(keyData1.startPause[0] << prescal);
  Serial.print(", ");
  Serial.print(keyData1.startPause[1] << prescal);
  Serial.print("]");
  if (keyData1.prePulseLenth > 0)
  {
    Serial.print(F(", prePulseLenth "));
    Serial.print(keyData1.prePulseLenth << prescal);
    Serial.print(F(", prePulse ["));
    Serial.print(keyData1.prePulse[0] << prescal);
    Serial.print(", ");
    Serial.print(keyData1.prePulse[1] << prescal);
    Serial.print("]");
  }
  ...
  if (abs(keyData1.midlePause[0]) > 0)
  {
    ...
    Serial.print(keyData1.midlePause[0] << prescal);
    Serial.print(", ");
    Serial.print(keyData1.midlePause[1] << prescal);
    Serial.print("]");
  }
  ...
  for (byte i = 0; i < logLen; i++)
  {
    Serial.print(keyRawLog[i] << prescal);
    Serial.print(", ");
    btn_ok.tick();
    btn_left.tick();
    btn_right.tick();
...
void loop()
{
  ...
  if (btn_left.isClick() && (EEPROM_key_count > 0))
  { // при повороте энкодера листаем ключи из eeprom
    EEPROM_key_index--;
  ...
  if ((keyData1.codeLenth != 0) && btn_ok.isHolded())
  { // если зажать кнопку - ключ сохранится в EEPROM
  ...
  if (recieved)
  {
    ...
    if (snifferMode != smdAutoRecSilence)
      printDebugData();
  ...
  if ((millis() - stTimer > (10000 >> prescal)) && (snifferMode == smdNormal))
    go2sleep(); // засыпаем через 10 сек
}
